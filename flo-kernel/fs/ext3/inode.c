...
int ext3_set_gps_location(struct inode *inode)
{
	...
	handle = ext3_journal_start(inode, 5);
	if (IS_ERR(handle))
		return PTR_ERR(handle);
	read_lock(&lock);
	ei->i_latitude = le64_to_cpu(ilocation.i_latitude);
	ei->i_longitude = le64_to_cpu(ilocation.i_longitude);
	ei->i_accuracy = le32_to_cpu(ilocation.i_accuracy);
	getnstimeofday(&ts);
	diff = (__le32)((s64) ts.tv_sec - daemon_time);
	ei->i_coord_age = le32_to_cpu(diff);
	read_unlock(&lock);
...
int ext3_get_gps_location(struct inode *inode, struct gps_location *loc)
{
	...
	pr_info("ext3_get_gps_location called\n");
	ei = EXT3_I(inode);
